...
client = jack.Client('NodeSynth2')
midiport = client.midi_inports.register('Midi Input')
audioport = client.outports.register('Audio Output')
...
def process(blocksize):
    ...
    for pitch, (envelope, _, _) in voices.items():
        ...
        tone = window.instrument.outNode.getValue(0, m2f(pitch) * t)
